            ...
            () -> new Block(BlockBehaviour.Properties.copy(Blocks.TNT)) {
                ...
                public void onPlace(BlockState pState, Level pLevel, BlockPos pPos, BlockState pOldState, boolean pIsMoving) {
                    if (!pLevel.isClientSide) {
                        GravityTntEntity tnt = new GravityTntEntity(pLevel, pPos.getX() + 0.5D, pPos.getY(), pPos.getZ() + 0.5D, null);
                        pLevel.addFreshEntity(tnt);
                        pLevel.playSound(null, tnt.getX(), tnt.getY(), tnt.getZ(), SoundEvents.TNT_PRIMED, SoundSource.BLOCKS, 1.0F, 0.5F);
