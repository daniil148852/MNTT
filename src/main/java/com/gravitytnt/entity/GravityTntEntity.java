...
public class GravityTntEntity extends PrimedTnt {
    ...
    public GravityTntEntity(EntityType<GravityTntEntity> pEntityType, Level pLevel) {
    ...
    public void tick() {
        if (!this.level().isClientSide) {
            ...
            for (Entity entity : entities) {
                ...
                if (distance > 1.5D) {
                    Vec3 pull = direction.normalize().scale(0.15D);
    ...
    protected void explode() {
        ...
        List<Entity> entities = this.level().getEntitiesOfClass(Entity.class, area, e -> e != this);
        for (Entity entity : entities) {
            entity.setDeltaMovement(entity.getDeltaMovement().add(0, 2.0D, 0));
            entity.hurtMarked = true;
        }
    }
}
